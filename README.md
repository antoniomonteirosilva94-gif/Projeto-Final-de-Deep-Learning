...
* **Restrições Quânticas:** Impor restrições de **ortogonalidade** ($\mathcal{L}_{\text{orto}}$) e **ordenação espectral** ($\mathcal{L}_{\text{ordenação}}$) para garantir a validade física das soluções[Monteiro, A: 106, 107].
...
| :--- | :--- | :--- |
| `src/models/dnn.py` | Implementação da classe `SpectrumSolver`. | [cite_start]Contém a **MLP Densa** e a lógica para o cálculo do resíduo da EDP e a imposição das CC via transformação $\eta_{n}(a)=a(L-a)\eta_{n}^{raw}(a)$[Monteiro, A: 97]. |
...
[cite_start]A **Figura 3 (Pacote de Ondas)** confirma a principal implicação física do tratamento quântico: a evolução do pacote de ondas mostra que o fator de escala do universo $a$ **nunca se anula**[Monteiro, A: 167]. As soluções são regulares (tipo buraco de minhoca) e eliminam a singularidade do Big Bang presente na dinâmica clássica[Monteiro, A: 9, 179].

---

## Referências

* MONTEIRO, A. et al. Cosmologia quântica computacional: aplicação do método espectral de galerkin no estudo da dinâmica do universo primitivo descrito
por radiação, constante cosmológica negativa e um potencial de pöschl-teller. Universidade do Estado do Rio de Janeiro, 2025.
